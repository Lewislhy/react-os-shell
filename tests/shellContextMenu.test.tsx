 ...
 * contenteditable keep Chrome's own menu, because its spellcheck suggestions
 * and "Add to dictionary" are not ours to rebuild and a Paste of our own would
 * need a clipboard permission prompt. A refactor that "tidies" the target test
 * would silently swallow spellcheck everywhere, and nothing on screen would
 * say so — hence a spec that dispatches real events and asserts on
 * `defaultPrevented`.
 *
 * The second rule is the one that lets 39 existing handlers stay untouched:
 * they all call preventDefault(), so the global listener must stand down on an
 * event that is already spoken for.
 */
...
test('a text input keeps Chrome\'s menu — not prevented, no shell menu', async () => {
...
test('a textarea keeps Chrome\'s menu', async () => {
...
test('a click on a child of a contenteditable keeps Chrome\'s menu', async () => {
...
test('shift+right-click falls through to Chrome, so Inspect stays reachable', async () => {
  const v = mount('<p id="p">Some prose.</p>');
...
test('an opted-out subtree keeps Chrome\'s menu', async () => {
