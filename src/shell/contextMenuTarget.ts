 ...
 * two rules that matter can be tested directly: which targets keep Chrome's
 * own menu, and which items a click produces.
 */
 ...
 *  so there is nothing in Chrome's menu worth keeping for it. */
const TEXT_INPUT_TYPES = new Set([
  'text', 'search', 'url', 'tel', 'email', 'password', 'number',
  'date', 'datetime-local', 'month', 'week', 'time',
]);

/** Opt-out attribute: anything inside `[data-native-context-menu]` keeps
 *  Chrome's menu. For surfaces the shell has no business re-skinning — an
...
/**
 * Does this target keep Chrome's native menu?
 *
 * Text inputs, textareas and contenteditable do, deliberately: Chrome's
 * spellcheck suggestions and "Add to dictionary" cannot be rebuilt by us, and
 * a Paste item of our own would need a clipboard permission prompt. That trade
 * was made explicitly — the shell menu is for everywhere else.
 */
 ...
 *
 * A linked image reports both, so the menu can offer both sections the way
 * Chrome's does; `kind` names whichever the menu should lead with.
 */
