 ...
 *
 * Right-click used to be a patchwork: fourteen surfaces in this package (and
 * twenty-five more in the admin portal) drew their own menu, and everywhere
 * else Chrome's native menu appeared, which reads as a hole in a desktop OS.
 * This component closes the hole with a single `contextmenu` listener on the
 * document, and does it without touching any of those handlers:
 *
 ...
 *     spellcheck suggestions and "Add to dictionary" are not ours to rebuild,
 *     and a Paste of our own would need a clipboard permission prompt.
 *   - Shift+right-click anywhere reaches Chrome's real menu, so "Inspect" is
 *     still one gesture away for developers.
 *
 * Mounted by `Layout`, so every portal gets it from a version bump alone. It is
...
export interface ShellContextMenuProps {
   ...
   *  Chrome. For a consumer that is mid-migration. */
  disabled?: boolean;
}
...
export default function ShellContextMenu({ disabled }: ShellContextMenuProps = {}) {
  ...
  useEffect(() => {
    ...
    const onContextMenu = (e: MouseEvent) => {
      ...
      // Developer escape hatch — Chrome's real menu, with Inspect on it.
